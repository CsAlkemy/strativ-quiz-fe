...
const Tabs = React.forwardRef<React.ElementRef<typeof TabsPrimitive.Root>, React.ComponentPropsWithoutRef<typeof TabsPrimitive.Root>>(
    ({ className, ...props }, ref) => (
        <TabsPrimitive.Root
            ...
            ref={ref}
            className={cvaCnMerge('flex rounded-md p-1 border-3 border-white text-muted-foreground gap-1', className)}
            {...props}
        />
    ),
);
...
const TabsList = React.forwardRef<React.ElementRef<typeof TabsPrimitive.List>, React.ComponentPropsWithoutRef<typeof TabsPrimitive.List>>(
    ({ className, ...props }, ref) => (
        <TabsPrimitive.List
            ref={ref}
            className={cvaCnMerge('flex flex-col h-auto items-center justify-start rounded-md bg-muted p-1 text-muted-foreground', className)}
            {...props}
        />
    ),
);
