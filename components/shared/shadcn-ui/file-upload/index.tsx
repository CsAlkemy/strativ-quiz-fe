        ...
        return (
            <FileUploaderContext.Provider
                ...
                <div
                    ...
                    className={cvaCnMerge('grid w-full focus:outline-none overflow-hidden ', className, {
...
export const FileUploaderContent = forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(({ children, className, ...props }, ref) => {
    ...
    return (
        <div className={cvaCnMerge('w-full px-1')} ref={containerRef} aria-description="content file holder">
            <div
                {...props}
                ref={ref}
                className={cvaCnMerge('flex rounded-xl gap-1', orientation === 'horizontal' ? 'flex-raw flex-wrap' : 'flex-col', className)}>
                {children}
            </div>
        </div>
    );
...
export const FileUploaderItem = forwardRef<HTMLDivElement, { index: number } & React.HTMLAttributes<HTMLDivElement>>(
    ({ className, index, children, ...props }, ref) => {
        ...
        return (
            <div
                ref={ref}
                className={cvaCnMerge('h-6 p-1 justify-between cursor-pointer relative', className, isSelected ? 'bg-muted' : '')}
                {...props}>
                <div className="font-medium leading-none tracking-tight flex items-center gap-1.5 h-full w-full">{children}</div>
                <button
                    type="button"
                    className={cvaCnMerge('absolute', direction === 'rtl' ? 'top-1 left-1' : 'top-1 right-1')}
